...
async def ping_self():
    ...
    url = f"https://six-o46c.onrender.com/health"  # Замените на ваш URL
    try:
...
async def get_original_message_text(message_id: int) -> str:
    """Получить оригинальный текст сообщения из группы"""
    try:
        # Получаем сообщение из группы
        message = await bot.forward_message(
            chat_id=bot.id,  # Пересылаем самому себе
            from_chat_id=TARGET_CHAT_ID,
            message_id=message_id
        )
        
        # Извлекаем текст без ссылок и форматирования
        text = message.text or message.caption or ""
        
        # Удаляем HTML теги и ссылки
        import re
        # Удаляем blockquote теги
        text = re.sub(r'<blockquote>(.*?)</blockquote>', r'\1', text, flags=re.DOTALL)
        # Удаляем ссылку на автора в конце
        text = re.sub(r'\n\n<a href="[^"]*">—</a>
...
async def ad_text_handler(message: Message, state: FSMContext):
    ...
    # Валидация текста
    is_valid, error_message = validate_message_text(ad_text)
    if not is_valid:
...
async def edit_ad_text_handler(message: Message, state: FSMContext):
    """Обработка текста для редактирования объявления"""
    # Блокируем из группы объявлений
    if message.chat.id == TARGET_CHAT_ID:
        return
    ...
    # Валидация текста
    is_valid, error_message = validate_message_text(new_text)
    if not is_valid:
        await message.answer(error_message)
        return
...
def validate_message_text(text: str) -> tuple[bool, str]:
    """Проверить текст сообщения на запрещенные элементы"""
    if not text:
        return False, "❌ Сообщение не может быть пустым!"
    
    # Проверяем на @username
    if '@' in text:
        return False, "❌ @username не принимаются, мы сами вставим ссылку на вас."
